...
# ╔═╡ 13720208-4c45-11ec-0d81-ed5bee4a4b1a
md" 
# Update"

# ╔═╡ a7789066-0fa6-4133-ae1f-5a0dc92eecad
md" 
# Inferring Seeding Locations"
...
Additionally, for each value of $n$, we test identifiability at 4 noise levels, given by: 
...
html"""
<img src="https://github.com/PavanChaggar/Notebooks/blob/main/status-update/images/fkpp-ec/fsl-pit-slow.png?raw=true" height=450 width=900>"""
...
# ╔═╡ 4d3a8387-f75f-433f-a6c0-c85c97f5d3b5
md"
τ = $(@bind τ Slider(0.1:0.1:10, show_value=true, default=1.0))
"

# ╔═╡ 2bbf55fa-c6ba-47d7-b3b6-2c5240be6dbf
begin 
...
md" ### $t_{n = 1}$
The posterior distributions in all seeding locations are inferred well for noise = 0, 0.02, 0.05. For noise = 0.1, posteriors are broader, but the initial seeding locations are identified." 

# ╔═╡ 7392c943-ceae-4d9a-b30b-e03b663ff25b
html"""
<img src="https://github.com/PavanChaggar/Notebooks/blob/main/status-update/images/seeding-ric/t1.png?raw=true" height=450 width=900>"""


# ╔═╡ 21357cec-5c52-4d6c-853d-1b3f43ae101e
md" ### $t_{n = 3}$
All but one seeding location (at node 78) are identified in the FSL connectome, with
broader distributions for higher noise levels. In the PIT connectome, all seeding locations and concentrations are identified with tight posteriors for noise = 0, 0.02. 
For noise = 0.05, all but one node is identified with tight posteriors. Node 13 is identified, however, the posterior mean for the concentration is considerably higher than the true value with high uncertainty. At noise = 0.1, only 5 nodes are identified  in the PIT connectome, each with significant uncertainty. 
"

# ╔═╡ 6c18cd9f-14ad-4a7a-b0cd-6596b4facbd0
html"""
<img src="https://github.com/PavanChaggar/Notebooks/blob/main/status-update/images/seeding-ric/t3.png?raw=true" height=450 width=900>"""


# ╔═╡ 3025c830-c67d-473c-ac1d-d95e4570f078
md" ### $t_{n = 5}$

There are deficiencies in identifiability for both the FSL and PIT connectome.

In the FSL connectome, for noise = 0, 0.02, 0.05, all but one node (node 78) are identified with good posterior uncertainty. For noise = 10, only 3 nodes are reliably identified, with other misidentified or with considerable posterior density still concentrated around 0.

In the PIT connectome, all non-zero noise levels, gives non-identifiability. In the best case, noise = 0, 7 nodes are reliably identified with good posteriors. for hgiher noise levels there are misidenfitied nodes and nodes with a high posterior density around 0."

# ╔═╡ fc98d796-425c-4930-8b37-f4ae34cb66a0
html"""
<img src="https://github.com/PavanChaggar/Notebooks/blob/main/status-update/images/seeding-ric/t5.png?raw=true" height=450 width=900>"""


# ╔═╡ 907e2994-6724-42f6-b1ca-7bd9ec31cadb
md"
# Next Questions: 
* Inferring $t_n$ from $t_{n + \epsilon}$. We can't guarantee _nice_ initial conditions like those at $t_0$. Instead, we will likely be inferring into a time point at which protein is already distributed through the brain with non-uniform values.
* Use volume corrected concentrations for inference. 
* Are network properties responsible for differences in identifiability?
"
